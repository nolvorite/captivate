...
if(isset($_SESSION['login_q']) && $swiss_army->compare_dz($logged_dt['salt'],$_SESSION['salt_q']) === true && $logged_dt['userid'] === "1"):

$admin_note_dt = $db_checks->quick_return("internal_settings","setting_name","'admin_notepad'");
$admin_note = mysqli_fetch_assoc($admin_note_dt);
?>
...
#panel_opts a.prompt:hover,#v_notepad a.prompt:hover{background:#333;color:#e0e0e0}   
...
#admin_panel button:focus,a.link_view2:focus,#admin_panel #panel_opts .selected_plink,#panel_opts a.selected_plink:hover,#v_notepad a.link_view2:focus{background:#79A8C8 url('<?php echo main_dir; ?>core/img/white.png') repeat-x;color:#fff;border:1px solid #658EAE;box-shadow: 0 0;text-shadow:0 1px 0 #658EAE}
...
";
  ...
  <div class="box"><textarea class="flick" id="sql_query"><?php echo $nx['sql_textbox_note']; ?></textarea>
  ...
  <div id="panel_opts"><a href="opts:minimize" class="prompt rad"><?php echo (isset($_SESSION['admin_panel']['minimized'])) ? $nx[82] : $nx[81]; ?></a><a href="opts:drag" class="prompt rad"><?php echo $nx[83];?></a></div>
...
$.get("<?php echo main_dir; ?>core/simcheck.php",{"fetch":"post_type_panel"},function(html_content){
...
$.get("<?php echo main_dir; ?>core/simcheck.php",{"action":"drag_box","screen_max":[screen.availWidth,screen.availHeight,$("#admin_panel").width(),$("#admin_panel").height()]}); 
