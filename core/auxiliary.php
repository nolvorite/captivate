...
if ( isset( $_SESSION['login_q'] ) && isset( $_SESSION['salt_q'] ) ) {
				$logged_query = mysqli_query( $db_main, "SELECT userid,salt,day2,month2,year2,tabbed_users,misc_prefs,fullname,login_att_last,profile_pic_url FROM userz WHERE username='$_SESSION[login_q]'" );
...
class lean_machines extends swiss_army {
				...
				public function get_posts( $pid_call, $tid_call, $limit = "25", $chain = "5", $special_select = "none", $show_more = "no" )
                 ...
                 AND thread_id='$tid_call' ORDER BY stamptime DESC LIMIT 0,$limit" ) or die( mysqli_error( $db_main ) );
																				...
																				$fill_data -> show_reply( $q_dt );
																				 if ( mysqli_num_rows( $inception_q ) > 0 ) {
																								$this -> get_posts( $q_dt['postid'], $tid_call, "25", $chain-1 );
																								 } 
																				echo $margins[1];
																				} 
																} 
        ...
        AND thread_id='$thread_data[postid]' ORDER BY stamptime DESC LIMIT 0,25" );
												 if ( mysqli_num_rows( $get_child_comments ) > 0 ) {
																echo mysqli_num_rows( $get_child_comments );
																 while ( $sync_children = mysqli_fetch_assoc( $get_child_comments ) ) {
																				$this -> get_posts( $sync_children['postid'], $thread_data['postid'] );
																				 } 
																mysqli_free_result( $get_child_comments );
																 } 
												} //end function
								 } 
				public function show_parents( $queue, $parent, $parent_comments = "5" )
												 ...
												 if ( $tell_2 !== "dead" && $_SESSION['reply_sync']['count'] > 0 ) {
																$_SESSION['reply_sync']['count'] = $_SESSION['reply_sync']['count'] - 1;
																 ...
																 if ( $tell_2['parent'] !== $thread_data['postid'] && !isset( $_SESSION['reply_sync']['has_more_parents'] ) ) {
																				$this -> show_parents( $tell_2['postid'], $tell_2['parent'], $_SESSION['reply_sync']['count'] - 1 );
        	...
        	public function key_associations( $key_names, $is_one_to_others = null ) // this limits which GET parameters you can have on any page to only appropriate associations.
								{
												// this is so tacky
												 ...
												 if ( count( $matches ) > 0 ) {
																foreach( $key_params as $keys ) {
																				if ( in_array( $keys, $key_names ) == false ) {
																								$this -> redir_process( "Location: " . main_dir );
																								 } 
																				} 
																} 
												unset( $key_params );
...
function chat_list_q( $status, $user ) // get list of users
{
				...
				global $db_main, $_MONITORED;
				 $logged_dt_again = mysqli_query( $db_main, "SELECT tabbed_users FROM userz WHERE username='$_MONITORED[login_q]'" );
				 $logged_dt_c = mysqli_fetch_assoc( $logged_dt_again );
         $logged_dt_c = preg_match("#^[{].+[}]$#",$logged_dt_c['tabbed_users']) ? preg_replace("#^([{])(.+)([}])$#","[$2]",$logged_dt_c['bywhom']) : $logged_dt_c;
				...
				switch ( $status ) {
								...
								case "open":
								 ...
								 case "close":
                         if(is_array($logged_dt_c) && isset($logged_dt_c[array_search( $user, $logged_dt_c )])){
				...
				$update_q = mysqli_query( $db_main, "UPDATE userz SET tabbed_users='$logged_dt_c' WHERE username='$_MONITORED[login_q]'" );
